...
max_boundary_dist = 0.10  # now in AlignmentParameters
...
gn_invalidate_too_far_away_translations = True  # Moved to TrainingParameters for now, since only used in train.py
gn_max_mean_translation_error = 0.5  # Moved to TrainingParameters for now, since only used in train.py
...
use_fixed_mask_loss_neg_wrt_pos_weight = True  # False if you don't wanna use a fixed weight, but to let it depend on the #negs vs # pos
