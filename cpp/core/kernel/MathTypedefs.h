...
open3d::core::Tensor EigenMatrixToTensor(const TEigenMatrix& matrix, const open3d::core::Device& device){
    ...
    if (TEigenMatrix::IsRowMajor) {
        return {matrix->data(), {row_count, col_count}, GetDType<TEigenMatrix::CoeffReturnType>()};
    } else {
        open3d::core::Tensor tensor(matrix->data(), {col_count, row_count}, GetDType<TEigenMatrix::CoeffReturnType>());
        return tensor.T();
    }
}
