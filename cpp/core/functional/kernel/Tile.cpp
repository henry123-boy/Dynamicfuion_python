...
void Tile(open3d::core::Tensor& tiled, const open3d::core::Tensor& source_tensor, int rows, int columns) {
	ExecuteOnDevice(
			source_tensor.GetDevice(),
			[&]() { Tile<open3d::core::Device::DeviceType::CPU>(tiled, source_tensor, rows, columns); },
			[&]() {
				NNRT_IF_CUDA(Tile<open3d::core::Device::DeviceType::CUDA>(tiled, source_tensor, rows, columns););
			}
	);
}
} // namespace nnrt::core::functional::kernel
