...
FactorizeBlockSparseArrowheadCholesky_Upper(const BlockSparseArrowheadMatrix& A) {
	...
	o3c::Tensor U_sparse_blocks = core::linalg::MatmulBlockSparseRowWise(L_inv_diagonal_upper_left, A.upper_blocks, A.upper_block_coordinates);
	...
	return std::make_tuple(U_diagonal_upper_left, U_sparse_blocks, U_factorized_dense_lower_right);
}
