...
NNRT_DEVICE_WHEN_CUDACC
inline tuple<core::kernel::Matrix3f, core::kernel::Matrix3f> Jacobian_PerspectiveCorrectBarycentricCoordinateWrtDistortedAndZ(
	...
	core::kernel::Matrix3f partial_coords_wrt_distorted;
	//@formatter:off
	...
	const float partial_denominator_wrt_z1 =
			distorted_barycentric_coordinates(0) * vertex2z + vertex0z * distorted_barycentric_coordinates(2);
    ...
    partial_coords_wrt_z <<
            ...
            denominator * distorted_barycentric_coordinates(0) * vertex2z -
            coord0_numerator * partial_denominator_wrt_z1,
            denominator * distorted_barycentric_coordinates(0) * vertex1z -
            coord0_numerator * partial_denominator_wrt_z2,

            denominator * distorted_barycentric_coordinates(1) * vertex2z -
            coord1_numerator * partial_denominator_wrt_z0,
            -coord1_numerator * partial_denominator_wrt_z1,
            denominator * distorted_barycentric_coordinates(1) * vertex0z -
            coord1_numerator * partial_denominator_wrt_z2,

            denominator * distorted_barycentric_coordinates(2) * vertex1z -
            coord2_numerator * partial_denominator_wrt_z0,
            denominator * distorted_barycentric_coordinates(2) * vertex0z -
...
NNRT_DEVICE_WHEN_CUDACC
inline core::kernel::Matrix3x9f Jacobian_BarycentricCoordinatesWrtCameraSpaceVertices_Generic(
	...
	for (int i_vertex = 0; i_vertex < 3; i_vertex++) {
		auto d_perspective_projection_wrt_vertex =
				CameraToNdcSpaceProjectionJacobian(ndc_focal_coefficient_x, ndc_focal_coefficient_y, vertices_camera_space[i_vertex]);
		d_barycentric_coordinates_d_vertices.block<3, 3>(0, i_vertex * 3) =
				d_barycentric_coordinates_d_ndc[i_vertex] * d_perspective_projection_wrt_vertex;
	}
...
inline core::kernel::Matrix3x9f Jacobian_BarycentricCoordinatesWrtCameraSpaceVertices_WithPerspectiveCorrection(
	...
	return Jacobian_BarycentricCoordinatesWrtCameraSpaceVertices_Generic(
			...
			[&ray_point, &distorted_barycentric_coordinates](
					const Eigen::Vector2f& ndc_vertex0,
			...
			[&distorted_barycentric_coordinates, &vertex0, &vertex1, &vertex2]
				...
				for (int i_coordinate = 0; i_coordinate < 3; i_coordinate++) {
					for (int i_vertex = 0; i_vertex < 3; i_vertex++) {
						d_barycentric_coordinates_d_vertices(i_coordinate,
						                                     i_vertex * 3 + 2) += d_perspective_corrected_d_z(
								i_coordinate, i_vertex);
					}
				}
			}
...
NNRT_DEVICE_WHEN_CUDACC
inline core::kernel::Matrix3x9f Jacobian_BarycentricCoordinatesWrtCameraSpaceVertices_WithoutPerspectiveCorrection(
		const TPoint& ray_point,
		const TVertex& vertex0,
		const TVertex& vertex1,
		const TVertex& vertex2,
	...
	return Jacobian_BarycentricCoordinatesWrtCameraSpaceVertices_Generic(
			ray_point, vertex0, vertex1, vertex2, perspective_projection,
			// Compute jacobian of barycentric coordinates w.r.t. NDC vertices
			[&ray_point, &distorted_barycentric_coordinates](
					const Eigen::Vector2f& ndc_vertex0,
					const Eigen::Vector2f& ndc_vertex1,
					const Eigen::Vector2f& ndc_vertex2
			) {
				return Jacobian_BarycentricCoordinateWrtNdcVertices<TVertexOrder>(ray_point, ndc_vertex0, ndc_vertex1,
				                                                                  ndc_vertex2,
				                                                                  distorted_barycentric_coordinates);
			},
			// Apply corrections. in this case, there are no corrections.
			[](core::kernel::Matrix3x9f& d_barycentric_coordinates_d_vertices) {}
	);
}
