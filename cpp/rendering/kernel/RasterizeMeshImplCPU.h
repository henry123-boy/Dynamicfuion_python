			...
			NNRT_LAMBDA_CAPTURE_CLAUSE NNRT_DEVICE_WHEN_CUDACC(int64_t workload_idx) {
				...
				for (int bin_y = 0; bin_y < grid_height_in_bins; bin_y++) {
					const float bin_y_min =
							ImageSpacePixelToNdc(bin_y * bin_side_length, image_height, image_width) - half_pixel_y;
					...
					if (y_overlap) {
						for (int bin_x = 0; bin_x < grid_width_in_bins; bin_x++) {
							const float bin_x_min =
									ImageSpacePixelToNdc(bin_x * bin_side_length, image_width, image_height) - half_pixel_x;
