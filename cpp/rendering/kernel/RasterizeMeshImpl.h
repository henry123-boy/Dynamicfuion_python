...
inline void InitializeFragments(
        Fragments& fragments, const o3c::Device& device,
    ...
    fragments.pixel_face_indices = o3c::Tensor::Full({image_height, image_width, faces_per_pixel}, -1, o3c::Int64,
                                                     device);
    fragments.pixel_depths = o3c::Tensor::Full({image_height, image_width, faces_per_pixel}, -1, o3c::Float32, device);
    fragments.pixel_barycentric_coordinates = o3c::Tensor::Full({image_height, image_width, faces_per_pixel, 3}, -1,
                                                                o3c::Float32, device);
    // float distance in the x/y camera plane in NDC coordinates, i.e. (-1,-1) to (1, 1), of each pixel-ray intersection
    // to faces_per_pixel of triangles closest to it along the z axis.
    fragments.pixel_face_distances = o3c::Tensor::Full({image_height, image_width, faces_per_pixel}, -1, o3c::Float32,
                                                       device);
}

template<open3d::core::Device::DeviceType TDeviceType, typename TFaceValidCheck>
void RasterizeMeshNaive_Generic(
    ...
    o3c::Device device = face_vertices_ndc.GetDevice();

    o3tgk::TArrayIndexer<t_face_index> face_vertex_position_indexer(face_vertices_ndc, 1);

    const auto face_count = static_cast<t_face_index>(face_vertices_ndc.GetLength());
    ...
    if (face_vertices_ndc.GetLength() == 0) {
        return;
    }
    ...
    o3c::ParallelFor(
            ...
            [=] OPEN3D_DEVICE(int64_t workload_idx) {
                ...
                for (int i_pixel_face = 0; i_pixel_face < queue_size; i_pixel_face++) {
                    ...
                    pixel_barycentric_coordinate_ptr[(fragment_index + i_pixel_face) * 3 +
                                                     0] = queue[i_pixel_face].barycentric_coordinates.x();
                    pixel_barycentric_coordinate_ptr[(fragment_index + i_pixel_face) * 3 +
                                                     1] = queue[i_pixel_face].barycentric_coordinates.y();
                    pixel_barycentric_coordinate_ptr[(fragment_index + i_pixel_face) * 3 +
                                                     2] = queue[i_pixel_face].barycentric_coordinates.z();
                }
            }
    );
}
    ...
    if (clipped_faces_mask.has_value()) {
        ...
        o3c::ParallelFor(
                ...
                NNRT_LAMBDA_CAPTURE_CLAUSE NNRT_DEVICE_WHEN_CUDACC(int64_t workload_idx) {
                    ...
                    CalculateAndStoreFace2dBoundingBox(face_bounding_box_data, face_skip_mask_data, workload_idx,
                                                       face_count, face_vertex0, face_vertex1, face_vertex2,
                                                       blur_radius_ndc);
