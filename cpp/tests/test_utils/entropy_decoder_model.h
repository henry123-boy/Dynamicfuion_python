...
template<unsigned long alphabet_size, typename entropy_decoder, unsigned long total_nodes, unsigned long order>
class entropy_decoder_model_mantissa {
	/*!
...
public:
	...
	explicit entropy_decoder_model_mantissa(entropy_decoder& coder);

	virtual ~entropy_decoder_model_mantissa();

	inline void clear();

	inline void decode(unsigned long& symbol);

	entropy_decoder& get_entropy_decoder() { return coder; }

	static unsigned long get_alphabet_size(
	) { return alphabet_size; }

	// restricted functions
	entropy_decoder_model_mantissa(entropy_decoder_model_mantissa<alphabet_size, entropy_decoder, total_nodes, order>&) = delete;        // copy constructor
	entropy_decoder_model_mantissa<alphabet_size, entropy_decoder, total_nodes, order>&
	operator=(entropy_decoder_model_mantissa<alphabet_size, entropy_decoder, total_nodes, order>&) = delete;    // assignment operator
...
template<
		...
		unsigned long order
>
entropy_decoder_model_mantissa<alphabet_size, entropy_decoder, total_nodes, order>::entropy_decoder_model_mantissa(entropy_decoder& coder_) :
		coder(coder_),
		next_node(1),
		cur_order(0),
		stack_size(0) {
	static_assert(1 < alphabet_size && alphabet_size < 65535, "Assertion failed.");
...
template<
		...
		unsigned long order
>
entropy_decoder_model_mantissa<alphabet_size, entropy_decoder, total_nodes, order>::~entropy_decoder_model_mantissa() {
	delete[] root;
}
...
template<unsigned long alphabet_size, typename entropy_decoder, unsigned long total_nodes, unsigned long order>
void entropy_decoder_model_mantissa<alphabet_size, entropy_decoder, total_nodes, order>::clear() {
	next_node = 1;
	root->child_context = 0;
	root->escapes = 0;
	root->total = 0;
...
template<
		...
		unsigned long order
>
void entropy_decoder_model_mantissa<alphabet_size, entropy_decoder, total_nodes, order>::
decode(unsigned long& symbol) {
	node* temp = cur;
	cur = 0;
		...
		if (space_left()) {
			...
			if (total_count > 0) {
				...
				while (true) {
					...
					if (high_count > target || n->next == nullptr)
						break;
					last = n;
					n = n->next;
				}
...
template<
		...
		unsigned long order
>
edmk5::node* entropy_decoder_model_mantissa<alphabet_size, entropy_decoder, total_nodes, order>::
allocate_node(
) {
	node* temp;
	temp = root + next_node;
...
template<
		...
		unsigned long order
>
bool entropy_decoder_model_mantissa<alphabet_size, entropy_decoder, total_nodes, order>::
space_left(
) const {
...
template<
		...
		unsigned long order
>
class entropy_decoder_model_exponent {
	/*!
		REQUIREMENTS ON total_nodes
			- 4096 < total_nodes
			- this is the total number of nodes that we will use in the tree
...
public:
	...
	explicit entropy_decoder_model_exponent(entropy_decoder& coder);

	virtual ~entropy_decoder_model_exponent();
	...
	// restricted functions
	entropy_decoder_model_exponent(entropy_decoder_model_exponent<alphabet_size, entropy_decoder, total_nodes, order>&) = delete;        // copy constructor
	entropy_decoder_model_exponent<alphabet_size, entropy_decoder, total_nodes, order>&
	operator=(entropy_decoder_model_exponent<alphabet_size, entropy_decoder, total_nodes, order>&) = delete;    // assignment operator
...
>
entropy_decoder_model_exponent<alphabet_size, entropy_decoder, total_nodes, order>::
entropy_decoder_model_exponent(entropy_decoder& coder_) :
		coder(coder_),
		next_node(1),
		cur_order(0),
		stack_size(0) {
...
template<
		...
		unsigned long order
>
entropy_decoder_model_exponent<alphabet_size, entropy_decoder, total_nodes, order>::~entropy_decoder_model_exponent() {
	delete[] root;
}
...
template<
		...
		unsigned long order
>
void entropy_decoder_model_exponent<alphabet_size, entropy_decoder, total_nodes, order>::
clear() {
	destroy_tree();
}
...
template<
		...
		unsigned long order
>
edmk4::node* entropy_decoder_model_exponent<alphabet_size, entropy_decoder, total_nodes, order>::allocate_node() {
	node* temp;
	temp = root + next_node;
	++next_node;
	return temp;
...
template<unsigned long alphabet_size, typename entropy_decoder, unsigned long total_nodes, unsigned long order>
bool entropy_decoder_model_exponent<alphabet_size, entropy_decoder, total_nodes, order>::
space_left() const {
	return (next_node < total_nodes);
}
...
template<
		...
		unsigned long order
>
edmk4::node* entropy_decoder_model_exponent<alphabet_size, entropy_decoder, total_nodes, order>::
pop() {
	--stack_size;
