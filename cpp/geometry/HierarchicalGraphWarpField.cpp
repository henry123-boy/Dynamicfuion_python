...
void HierarchicalGraphWarpField::RebuildRegularizationLayers(int count, int max_vertex_degree) {
	...
	for (int i_layer = 1; i_layer < count; i_layer++) {
		...
		o3c::Tensor previous_layer_virtual_node_indices = o3c::Tensor::Arange(running_node_count, running_node_count + previous_layer.node_indices.GetLength(), 1, o3c::Int32, device);
		running_node_count += updated_previous_layer_node_count;
		...
		kernel::warp_field::PrepareLayerEdges(layer_edges, max_vertex_degree, previous_layer_unfiltered_bin_node_indices,
		                                      previous_layer_virtual_node_indices);
		...
		previous_layer.edges = layer_edges;
