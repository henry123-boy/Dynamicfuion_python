...
def main():
    ...
    original_mesh: o3d.geometry.TriangleMesh = volume.extract_surface_mesh(-1, 0).to_legacy()
    renderer = PyTorch3DRenderer((depth_image.rows, depth_image.columns), device, intrinsics_open3d_cuda)

    frame_count = 6
    offset_increment = 0.01

    def offset_mesh_plus_xy(mesh: o3d.geometry.TriangleMesh, offset: float) -> o3d.geometry.TriangleMesh:
    ...
    # prepare folders
    root_output_directory = os.path.join(PathParameters.output_directory.value, "berlin_xy_offset_sequence")
    ...
    for i_frame in range(0, frame_count):
        offset_mesh = offset_mesh_plus_xy(original_mesh, offset_increment * i_frame)
