...
def Backward(x, flow):
    if str(flow.size()) not in backward_grid:
        tensorHorizontal = torch.linspace(-1.0, 1.0, flow.size(3), device=x.device).view(1, 1, 1, flow.size(3)).expand(flow.size(0), -1, flow.size(2),
                                                                                                                       -1)
        tensorVertical = torch.linspace(-1.0, 1.0, flow.size(2), device=x.device).view(1, 1, flow.size(2), 1).expand(flow.size(0), -1, -1,
                                                                                                                     flow.size(3))
    ...
    def __init__(self):
        ...
        class Extractor(torch.nn.Module):
            ...
            def forward(self, x):
                ...
                assert torch.max(x) <= 1.0, "input image is not within the range [0, 1], but rather within [0, {:f}]".format(torch.max(x))
        ...
        class Decoder(torch.nn.Module):
            ...
            def forward(self, first, second, object_prev):
                ...
                if object_prev is None:
                    ...
                    cost_volume = torch.nn.functional.leaky_relu(input=correlation.FunctionCorrelation(first=first, second=second),
                    ...
                    cost_volume = torch.nn.functional.leaky_relu(input=correlation.FunctionCorrelation(first=first, second=second_warped),
                                                                 negative_slope=0.1, inplace=False)
