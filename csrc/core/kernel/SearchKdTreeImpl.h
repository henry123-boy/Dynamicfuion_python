...
NNRT_DEVICE_WHEN_CUDACC
inline void
FindKnnInKdSubtreeRecursive_PriorityQueue2(const KdTreeNode* node, NearestNeighborHeap<TDeviceType>& nearest_neighbor_heap,
	...
	if (search_left_first) {
		if (search_left) {
			FindKnnInKdSubtreeRecursive_PriorityQueue2<TDeviceType>(node->left_child, nearest_neighbor_heap, query_point,
			                                                        kd_tree_point_indexer,
			                                                        (i_dimension + 1) % dimension_count, dimension_count, make_point_vector);
		}
		if (search_right) {
			FindKnnInKdSubtreeRecursive_PriorityQueue2<TDeviceType>(node->right_child, nearest_neighbor_heap, query_point,
			                                                        kd_tree_point_indexer,
			                                                        (i_dimension + 1) % dimension_count, dimension_count, make_point_vector);
		}
	} else {
...
template<open3d::core::Device::DeviceType TDeviceType, typename TPointVector, typename TMakePointVector>
NNRT_DEVICE_WHEN_CUDACC
inline void FindKnnInKdSubtreeIterative_PriorityQueue2(const KdTreeNode* root,
                                                       NearestNeighborHeap<TDeviceType>& nearest_neighbor_heap,
                                                       const TPointVector& query_point,
                                                       const o3gk::NDArrayIndexer& kd_tree_point_indexer,
                                                       TMakePointVector&& make_point_vector) {
