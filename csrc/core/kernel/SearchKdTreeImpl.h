...
inline void
FindKnnInKdSubtreeRecursive(const KdTreeNode* node, const KdTreeNode* last_node_to_check, NearestNeighborHeap<TDeviceType>& nearest_neighbor_heap,
	...
	if (search_left_first) {
		if (search_left) {
			FindKnnInKdSubtreeRecursive<TDeviceType>(node->left_child, last_node_to_check, nearest_neighbor_heap, query_point, kd_tree_point_indexer,
			                                         (i_dimension + 1) % dimension_count, dimension_count, make_point_vector);
		}
		if (search_right) {
			FindKnnInKdSubtreeRecursive<TDeviceType>(node->right_child, last_node_to_check, nearest_neighbor_heap, query_point, kd_tree_point_indexer,
			                                         (i_dimension + 1) % dimension_count, dimension_count, make_point_vector);
		}
	} else {
		if (search_right) {
...
template<open3d::core::Device::DeviceType TDeviceType, typename TPointVector, typename TMakePointVector>
NNRT_DEVICE_WHEN_CUDACC
inline void FindKnnInKdSubtreeIterative(const KdTreeNode* root, const KdTreeNode* last_node_to_check,
...
template<open3d::core::Device::DeviceType TDeviceType, typename TPointVector, typename TMakePointVector,
		typename TGetMaxDistance,
		typename TReplaceMaxDistanceEntry>
NNRT_DEVICE_WHEN_CUDACC
inline void FindKnnInKdSubtreeIterative_Generic(const KdTreeNode* root, const KdTreeNode* last_node_to_check,
		// NearestNeighborHeap<TDeviceType>& nearest_neighbor_heap,
		                                        const TPointVector& query_point,
		                                        const o3gk::NDArrayIndexer& kd_tree_point_indexer,
		                                        TMakePointVector&& make_point_vector, TGetMaxDistance&& get_max_distance,
		                                        TReplaceMaxDistanceEntry&& replace_max_distance_entry) {
	// Allocate traversal stack from thread-local memory,
	// and push nullptr onto the stack to indicate that there are no deferred nodes.
	const KdTreeNode* stack[64];
	const KdTreeNode** stack_cursor = stack;
	*stack_cursor = nullptr; // push nullptr onto the bottom of the stuck
	...
	do {
		...
		if (node <= last_node_to_check) {
			...
			// update the nearest neighbor heap if the distance is better than the greatest nearest-neighbor distance encountered so far
			if (max_knn_distance > node_distance) {
				replace_max_distance_entry(node_distance, node->index);
...
template<open3d::core::Device::DeviceType TDeviceType, SearchStrategy TSearchStrategy, typename TMakePointVector>
inline void
	...
	launcher::ParallelFor(
			...
			[=] OPEN3D_DEVICE(int64_t workload_idx) {
				...
				if (TSearchStrategy == SearchStrategy::RECURSIVE) {
					FindKnnInKdSubtreeRecursive<TDeviceType>(root_node, last_node_to_check, nearest_neighbor_heap, query_point, kd_tree_point_indexer,
					                                         0, dimension_count, make_point_vector);
				} else {
					FindKnnInKdSubtreeIterative_Generic<TDeviceType>(root_node, last_node_to_check, /*nearest_neighbor_heap,*/ query_point,
					                                         kd_tree_point_indexer,
					                                         make_point_vector,
					                                         [&nearest_neighbor_heap]() { return nearest_neighbor_heap.Head().distance; },
					                                         [&nearest_neighbor_heap](const float node_distance, const int index) {
						                                         nearest_neighbor_heap.Pop();
						                                         nearest_neighbor_heap.Insert(KdDistanceIndexPair{node_distance, index});
					                                         });
					// FindKnnInKdSubtreeIterative<TDeviceType>(root_node, last_node_to_check, nearest_neighbor_heap, query_point,
					//                                                  kd_tree_point_indexer, make_point_vector);
				}
