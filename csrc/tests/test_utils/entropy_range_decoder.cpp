...
#include "entropy_range_decoder.h"


namespace test {


entropy_range_decoder::entropy_range_decoder() :
...
void entropy_range_decoder::clear() {
	in = 0;
	r = 0;
	low = initial_low;
	high = initial_high;
...
void entropy_range_decoder::set_stream(std::istream& in_) {
	r = 0;
	low = initial_low;
	high = initial_high;
	target = 0x00000000;
...
// ----------------------------------------------------------------------------------------
bool entropy_range_decoder::stream_is_set() const {
	return in != nullptr;
}

// ----------------------------------------------------------------------------------------
std::istream& entropy_range_decoder::get_stream() const {
	return *in;
}

// ----------------------------------------------------------------------------------------
void entropy_range_decoder::decode(uint32 low_count, uint32 high_count) {
...
bool entropy_range_decoder::get_target_called() const {
	return (r != 0);
}

// ----------------------------------------------------------------------------------------
uint32 entropy_range_decoder::get_target(uint32 total) {
