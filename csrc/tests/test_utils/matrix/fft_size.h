...
namespace test
{
    ...
    inline test::uint32 hash(
        const fft_size& item,
        test::uint32 seed = 0)
    {
        seed = test::hash((test::uint64)item.num_dims(), seed);
        seed = std::accumulate(item.begin(), item.end(), seed, [](test::uint32 seed, long next) {
            return test::hash((test::uint64)next, seed);
        });
        return seed;
    }
    /*!
