...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        try
        {
            ...
            if (NR != 0 && nr != NR)
                throw serialization_error("Error while deserializing a test::matrix.  Invalid rows");
            if (NC != 0 && nc != NC)
                throw serialization_error("Error while deserializing a test::matrix.  Invalid columns");
        ...
        {
            throw serialization_error(e.info + "\n   while deserializing a test::matrix");
        }
    }
