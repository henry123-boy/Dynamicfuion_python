...
// messages.
    ...
    #define ASSERT_ARE_SAME_TYPE(type1, type2) static_assert(::test::assert_types_match<type1,type2>::value, "These types should be the same but aren't.")
    #define ASSERT_ARE_NOT_SAME_TYPE(type1, type2) static_assert(!::test::assert_types_match<type1,type2>::value, "These types should NOT be the same.")
#else
    #define COMPILE_TIME_ASSERT(expression) \
        DLIB_NO_WARN_UNUSED typedef char DLIB_BOOST_JOIN(DLIB_CTA, __LINE__)[::test::compile_time_assert<(bool)(expression)>::value]

    #define ASSERT_ARE_SAME_TYPE(type1, type2) \
        DLIB_NO_WARN_UNUSED typedef char DLIB_BOOST_JOIN(DLIB_AAST, __LINE__)[::test::assert_are_same_type<type1,type2>::value]

    #define ASSERT_ARE_NOT_SAME_TYPE(type1, type2) \
        DLIB_NO_WARN_UNUSED typedef char DLIB_BOOST_JOIN(DLIB_AANST, __LINE__)[::test::assert_are_not_same_type<type1,type2>::value]
#endif
...
#define DLIBM_CASSERT(_exp,_message)                                              \
    ...
    {                                                                       \
        ...
        throw test::fatal_error(test::EBROKEN_ASSERT,dlib_o_out.str());      \
    }}                                                                      
