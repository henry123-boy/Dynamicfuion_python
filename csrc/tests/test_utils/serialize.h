...
/*!
    ...
    short, int, long, unsigned short, unsigned int, unsigned long, and test::uint64
...
namespace test
{
    ...
    typename enable_if<is_protocol_buffer<T> >::type serialize(const T& item, std::ostream& out)
    {
        ...
        if (!item.SerializeToString(&temp))
            throw test::serialization_error("Error while serializing a Google Protocol Buffer object.");
        if (temp.size() > std::numeric_limits<uint32>::max())
            throw test::serialization_error("Error while serializing a Google Protocol Buffer object, message too large.");
    ...
    typename enable_if<is_protocol_buffer<T> >::type deserialize(T& item, std::istream& in)
    {
        ...
        if (!in || size == 0)
            throw test::serialization_error("Error while deserializing a Google Protocol Buffer object.");
...
// ----------------------------------------------------------------------------------------
    ...
    {                                                                   \
        ...
        catch (test::serialization_error& e)                            \
        {                                                               \
            throw test::serialization_error(e.info + "\n   while serializing object of type " #Type); \
        }                                                               \
    }                                                                   \
                                                                        \
