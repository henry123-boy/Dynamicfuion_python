...
//@formatter:on
				...
				for (int i_coefficient = 0; i_coefficient < 8; i_coefficient++) {
					coefficients[i_coefficient] = 0.0f;
				}
				for (int i_anchor = 0; i_anchor < anchor_count; i_anchor++) {
				...
				Eigen::Vector3f warped_voxel = voxel_transformation.transformPoint(voxel_camera);
				if (warped_voxel.z() < 0) {
...
//@formatter:on
				...
				for (int i_anchor = 0; i_anchor < anchor_count; i_anchor++) {
					...
					if(anchor_node_index != -1){
						...
						Eigen::Vector3f node(node_pointer[0], node_pointer[1], node_pointer[2]);
						warped_voxel += anchor_weight * (node + node_rotation * (voxel_global_metric - node) + node_translation);
					}
				}
