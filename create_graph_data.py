...
def save_graph_data(seq_dir: str, pair_name: str, node_coords: np.ndarray, graph_edges: np.ndarray,
    ...
    output_graph_nodes_path = os.path.join(dst_graph_nodes_dir, pair_name + "_{}_{:.2f}.bin".format("geodesic", options.node_coverage))
    output_graph_edges_path = os.path.join(dst_graph_edges_dir, pair_name + "_{}_{:.2f}.bin".format("geodesic", options.node_coverage))
    output_graph_edges_weights_path = os.path.join(dst_graph_edges_weights_dir,
                                                   pair_name + "_{}_{:.2f}.bin".format("geodesic", options.node_coverage))
    output_node_deformations_path = os.path.join(dst_node_deformations_dir, pair_name + "_{}_{:.2f}.bin".format("geodesic", options.node_coverage))
    output_graph_clusters_path = os.path.join(dst_graph_clusters_dir, pair_name + "_{}_{:.2f}.bin".format("geodesic", options.node_coverage))

    output_pixel_anchors_path = os.path.join(dst_pixel_anchors_dir, pair_name + "_{}_{:.2f}.bin".format("geodesic", options.node_coverage))
    output_pixel_weights_path = os.path.join(dst_pixel_weights_dir, pair_name + "_{}_{:.2f}.bin".format("geodesic", options.node_coverage))
...
def check_graph_data_against_ground_truth(seq_dir: str, ground_truth_pair_name: str,
    ...
    gt_output_graph_nodes_path = os.path.join(dst_graph_nodes_dir,
                                              ground_truth_pair_name + "_{}_{:.2f}.bin".format("geodesic", options.node_coverage))
