...
def main() -> int:
    # === preprocess options & initialize structures ===
    extracted_framewise_canonical_mesh_needed = \
    ...
    framewise_warped_mesh_needed = \
        po.source_image_mode != po.SourceImageMode.REUSE_PREVIOUS_FRAME or \
        po.visualization_mode == po.VisualizationMode.WARPED_MESH or \
        po.record_warped_meshes_to_disk

    telemetry_generator = TelemetryGenerator(po.record_visualization_to_disk,
    ...
    while sequence.has_more_frames():
        ...
        if current_frame.frame_index == sequence.start_frame_index:
            ...
            if extracted_framewise_canonical_mesh_needed:
                canonical_mesh = volume.extract_surface_mesh(0).to_legacy_triangle_mesh()
